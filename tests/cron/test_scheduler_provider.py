...
def test_fire_due_default_claims_then_runs(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda job, **kw: ran.append(
            (job["id"], job["fire_claim"]["by"], kw.get("fire_provenance"))
        )
        or True,
    )
    ...
    # The provider fire keeps the fork's external_provider provenance so a
    # Chronos-driven fire never inherits scheduled/manual authority.
    assert ran == [("j1", "exact-owner", "external_provider")]
