...
# Production renders this as str(get_hermes_home()) + "/profiles/<name>/", and
# str(Path("/hermes")) is platform-dependent (backslash on Windows) — build the
# expectation the same way instead of hardcoding "/hermes".
_EXPECTED_PROFILE_BLOCK = (
    "Active Hermes profile: default. Other profiles (if any) live "
    f"under {Path('/hermes')}/profiles/<name>/. Each profile has its own "
    "skills/, plugins/, cron/, and memories/ that affect a different session "
    "than this one. Do not modify another profile's skills/plugins/cron/"
    "memories unless the user explicitly directs you to."
)
