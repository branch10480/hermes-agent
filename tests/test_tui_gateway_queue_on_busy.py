...
def test_busy_image_prompts_keep_b_and_c_attachments_in_submission_order(monkeypatch):
    ...
    # ``direct_user_message`` is the fork's direct-user provenance boundary:
    # ``_drain_queued_prompt`` always forwards ``queued.get(...)``, which is
    # None for a gateway-submitted prompt (only a submission whose direct text
    # equals the prompt text stamps it).
    assert dispatched == [
        (
            "drain-b",
            "sid",
