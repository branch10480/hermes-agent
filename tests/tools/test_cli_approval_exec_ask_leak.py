...
class TestExecuteCodeGuardCliDenialBreakerParity:
    ...
    tripped, the LOCKOUT stage refuses further dangerous-operation candidates
    outright — ahead of the guardian call and ahead of the CLI panel — rather
    than prompting again with an escalated note attached.
    """
    ...
    def test_lockout_refuses_without_reaching_the_cli_panel(self, monkeypatch):
        """Once the lockout is active, the CLI panel is never shown.

        The lockout stage exists to stop a retry loop from burning a guardian
        LLM call — and a human's attention — on every variant of a command the
        reviewer has already refused three times in a row. So the refusal is
        issued by ``_denial_lockout_deny`` before either of those, and it is
        attributed to the guardian's standing verdict: nobody was asked, so
        the outcome cannot be a user deny or a prompt timeout.
        """
        monkeypatch.setenv("HERMES_EXEC_ASK", "1")
        ...
        def _panel(command, description, **kwargs):
            prompted.append(command)
            return "timeout"

        set_approval_callback(_panel)
        result = check_execute_code_guard("print('hi')", "local")

        assert not prompted, (
            "the lockout must refuse before the Dangerous Command panel: "
            "prompting again is the cost this stage exists to avoid"
        )
        assert result.get("approved") is False
        assert result.get("denial_lockout") is True
        assert result.get("outcome") == "denied"
        assert (
            result.get("approval_outcome")
            == approval_module.APPROVAL_OUTCOME_GUARDIAN_DENIED
        )
        # Nobody was asked, so there is no human consent to record either way.
        assert result.get("user_consent") is None

        message = result.get("message") or ""
        # Not compared against ``expected`` verbatim: the refusal counts
        # itself toward the hard stop before rendering, so its addendum is one
        # attempt further along than the one computed above. The stage marker
        # is the stable part.
        assert "DANGEROUS-OPERATION LOCKOUT:" in message
        assert "nobody has refused it" in message, (
            "the message must not read as a human denial — no human saw it"
        )

    def test_lockout_carries_v3_breaker_metadata(self, monkeypatch):
        """The refusal names which stage fired, so a reader can tell the
        turn-continuing lockout from the turn-ending hard stop."""
        monkeypatch.setenv("HERMES_EXEC_ASK", "1")
        session_key = approval_module.get_current_session_key()
        threshold = approval_module._get_denial_breaker_threshold()
        for _ in range(threshold):
        ...
        metadata = result.get(approval_module.APPROVAL_BREAKER_METADATA_KEY)
        assert metadata is not None, "a lockout refusal carries breaker metadata"
        assert metadata["version"] == 3
        assert metadata["mode"] == "lockout"
        assert metadata["threshold"] == threshold
        assert metadata["hard_stop_threshold"] == (
            threshold + approval_module._get_denial_breaker_lockout_attempts()
        )
