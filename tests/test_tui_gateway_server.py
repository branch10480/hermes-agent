...
def test_prompt_submit_fails_open_inline_when_compute_host_dispatch_breaks(monkeypatch):
    ...
    _rid, _sid, _text, _kwargs = inline_calls[0]
    assert (_rid, _sid, _text) == ("fallback-turn", "iso-fallback", "hello")
    # Direct-user provenance must survive the compute-host fail-open path.
    # Asserted on the one key rather than the whole kwargs dict: the inline
    # call also carries upstream's display_kind / terminal_callback, and this
    # test is about provenance, not the full signature.
    assert _kwargs["direct_user_message"] == "hello"
...
def test_run_prompt_submit_never_ticks_after_message_complete(monkeypatch):
    ...
    class _Agent:
        def run_conversation(
            ...
            # A plain typed submit also carries the fork's direct-user
            # provenance kwargs (direct_user_message[_provenance]); without
            # **kwargs the turn would die on a TypeError before any tick.
            **_kwargs,
        ):
