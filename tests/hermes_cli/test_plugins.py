...
class TestPreToolCallModify:
    ...
    def test_dispatch_forwards_direct_user_authority(self, monkeypatch):
        """The fork's provenance kwargs must reach the hook and must be accepted.

        Every dispatch site (model_tools / tool_executor / agent_runtime_helpers)
        passes them inside a bare ``except Exception``, so a signature drift
        here disables *all* pre_tool_call blocking silently. Regression guard
        for the v2026.8.31 merge (resolve_pre_tool_block ->
        _dispatch_pre_tool_call_hooks rename).
        """
        seen = {}

        def _invoke(hook_name, **kwargs):
            seen.update(kwargs)
            return [{"action": "block", "message": "no"}]

        monkeypatch.setattr("hermes_cli.plugins.invoke_hook", _invoke)
        block_msg, _modified = _dispatch_pre_tool_call_hooks(
            "write_file",
            {"path": "x"},
        ...
        assert block_msg == "no"
        assert seen["direct_user_authority_revision"] == 7
        assert seen["direct_user_authority_kind"] == "direct_text"
