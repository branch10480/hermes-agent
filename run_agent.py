...
class AIAgent:
    ...
    def _spawn_background_review(
        ...
        # ``focus`` distinguishes the two callers by SENTINEL, not by None: an
        # automatic post-turn trigger omits the argument entirely, while a
        # no-argument manual ``/refine`` passes an explicit ``focus=None``.
        # Every "is this automatic?" gate below therefore tests ``automatic``,
        # never ``focus is None``.
        ...
        # inflating token cost (#85859). An explicit ``/refine`` is a
        # deliberate user request and still runs.
        ...
        def _review_worker(review_run: Any) -> None:
            """Build and run the review on the worker thread.

            Deferred so the per-review run token can be minted at the moment
            the thread actually starts (see ``_start_review_thread``); prompt
            assembly is pure string work, so moving it off the foreground
            costs nothing.
            """
        ...
        # session context that the review needs for its writes (#54937).
        wrapped_target = propagate_context_to_thread(_review_worker)
        ...
        def _start_review_thread() -> None:
            # Mint the run token at the LAST moment, not when the review is
            # queued. A review parked behind the idle gate must not hold a run
            # in flight: the next live turn's
            # ``cancel_background_review_for_live_turn`` would wait out its
            # full acknowledgement deadline for a review that never started,
            # and cancelling the parked timer would strand the token forever,
            # fencing off every later review.
            review_run = prepare_background_review_run(self)
            if review_run is None:
                return
            try:
                # ``target=`` only, no ``args=``: the run token rides a closure
                # so every Thread stub in the suite (and any host that patches
                # threading.Thread) keeps its keyword-only signature.
                t = threading.Thread(
                    target=lambda: wrapped_target(review_run),
                    daemon=True,
                    name="bg-review",
                )
                t.start()
