...
| `on_context_pressure` | Directive/control | Before each fully assembled provider request and before any preflight compression decision; all valid string/`{"context": ...}` returns are joined and injected into the active user message in the API-only request copy. | `session_id`, `task_id`, `turn_id`, `approx_tokens`, `threshold_tokens`, `context_length`, `conversation_history`, `model`, `platform` | Full conversation history plus approximate context-pressure metadata. |
| `pre_context_compression` | Observer | After durable parent adoption and immediately before memory-provider/compressor work; return ignored and failures are fail-open. | `session_id`, `task_id`, `approx_tokens`, `threshold_tokens`, `conversation_history`, `model`, `platform` | Exact transcript selected for compression and model/routing metadata. |
| `post_context_compression` | Observer | Only after an in-place or rotated compression boundary commits successfully; return ignored. | `session_id`, `old_session_id`, `task_id`, `in_place`, `compression_count`, `model`, `platform` | Session lineage, compression count, and routing metadata; no message body. |
...
| `filter_api_tools` | Transformer | Per provider attempt, after the tool list for the request is chosen; return an iterable of tool names to omit from the wire request (or `None`). | `session_id`, `task_id`, `platform`, `model`, `tool_names` |
...
Fires once per provider attempt with `tool_names`, the names of the tool schemas about to be serialized for this request. Return an iterable of names to omit from the wire request, or `None` to keep every tool. Only the request payload changes: `agent.tools`, tool execution and the `pre_tool_call` gate are untouched, so a hidden tool the model still names is validated and gated exactly as before. Decide per session (for example by `model` or `platform`) rather than per call, so the serialized tool prefix stays stable for prompt caching. The hook is fail-open: a missing hook, an unrecognized return value or an exception keeps the full list. Use it when a role can never call a tool anyway and its schema only costs prompt tokens.
