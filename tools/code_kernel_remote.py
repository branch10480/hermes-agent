...
def execute_in_remote_kernel(
    ...
    *,
    ...
    # Per-cell approval-denial breaker bridge. The thread below carries this
    # cell's context, so the poll loop can read the side channel directly —
    # no probe indirection is needed here, unlike the local session kernel
    # whose serving thread outlives the cell.
    ...
    if cell_status in ("timeout", "protocol-error", "no-result"):
        ...
        _attach_approval_breaker_result(aborted, approval_breaker_state)
        return aborted
