...
class CellAuthority:
    ...
    def __init__(self, task_id: str, approval_breaker_state: Optional[dict] = None):
        import contextvars

        self.task_id = task_id
        self.ctx = contextvars.copy_context()
        self.active = True
        # This cell's approval-denial breaker bridge. Per-cell like the rest
        # of the authority: the serving thread outlives many cells, so the
        # state a nested denial latches must not leak into the next one.
        self.approval_breaker_state = approval_breaker_state
    ...
    def probe_approval_breaker(self):
        """Read the approval-denial breaker under THIS cell's context.

        ``_sync_approval_breaker_state`` binds the outer execute_code call's
        identity (session key, turn id, tool-call id) from contextvars on
        first use. The kernel's serving thread carries none of that, so the
        read has to happen inside the cell's captured context — the same
        reason ``dispatch`` does. Returns the trip metadata once the breaker
        has fired for this cell, else None.
        """
        if not self.active or self.approval_breaker_state is None:
            return None
        from tools.code_execution_tool import _sync_approval_breaker_state

        return self.ctx.run(
            _sync_approval_breaker_state, self.approval_breaker_state
        )
    ...
    def _invoke(self, tool_name: str, tool_args: dict) -> str:
        ...
        from tools.code_execution_tool import (
            _approval_breaker_skip_result,
            _sync_approval_breaker_state,
        )

        # Last line of defence for the breaker. The serving loop already
        # probes ahead of the call (that is the arm that keeps a suppressed
        # call out of the tool-call count); this one closes the race where a
        # nested denial trips between that probe and this dispatch, and
        # covers any caller reaching dispatch() without the loop's probe.
        # Called directly, NOT via self.ctx.run: _invoke is already running
        # inside self.ctx, and re-entering a live context raises.
        if self.approval_breaker_state is not None:
            metadata = _sync_approval_breaker_state(self.approval_breaker_state)
            if metadata is not None:
                return _approval_breaker_skip_result(metadata)
        ...
        try:
            ...
            if previous is not None and self._callback_setters is not None:
                ...
                try:
                    ...
                    pass
            # Latch a trip raised by THIS call while its identity is still
            # bound, so eviction or a session reset between now and the next
            # probe cannot reopen dispatch.
            if self.approval_breaker_state is not None:
                _sync_approval_breaker_state(self.approval_breaker_state)
...
def _rpc_forever(kernel: SessionKernel, max_tool_calls: int,
    ...
    def _breaker_probe():
        ...
        if authority is None:
            return None
        return authority.probe_approval_breaker()
...
def execute_in_session_kernel(
    ...
    *,
    ...
    #
    # The breaker bridge is per-cell for the same reason: it latches the
    # first nested denial that trips the breaker and must not carry that
    # latch into the next cell.
    approval_breaker_state: dict = {}
    authority = CellAuthority(
        task_id, approval_breaker_state=approval_breaker_state
    )
    ...
    with kernel.lock:
        try:
            ...
            _attach_approval_breaker_result(failure, approval_breaker_state)
