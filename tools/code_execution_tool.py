...
def _rpc_server_loop(
    ...
    ``breaker_probe`` overrides how the approval-denial breaker is read, for
    the same reason. ``_sync_approval_breaker_state`` binds the outer call's
    identity from contextvars, so reading it on THIS thread is only correct
    when the thread carries the cell's context. A session kernel's serving
    thread does not, so it passes a probe that runs the read inside the
    CURRENT cell's context instead. The read must stay here — ahead of the
    dispatch and of ``tool_call_counter``'s increment — so a suppressed call
    is never counted as made (#96873 regression: routing every local run
    through the session kernel silently bypassed this gate entirely).
    """
    ...
    def read_breaker():
        if breaker_probe is None:
            return _sync_approval_breaker_state(approval_breaker_state)
        try:
            return breaker_probe()
        except Exception:
            # Same fail-open-on-inspection-error contract as
            # _sync_approval_breaker_state: a broken probe must not wedge
            # every nested tool call. Suppression rides on the side channel,
            # which a later probe re-reads.
            logger.debug(
                "Approval-breaker probe failed", exc_info=True
            )
            return None
...
def _finish_remote_kernel_result(kernel_result: Dict[str, Any], *,
    ...
    # Carry the breaker marker the remote cell attached. Diagnostic only —
    # the executor's suppression still rides the in-process side channel —
    # but dropping it here would make the remote path the one place a
    # tripped breaker is invisible to the model and the gateway.
    if kernel_result.get("approval_breaker") is not None:
        result["approval_breaker"] = kernel_result["approval_breaker"]
