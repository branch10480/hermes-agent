...
def _try_dispatch_background_run(
    ...
    # Placed AFTER the stale-execution reap on purpose: the one-shot CLI run
    # this guard sends down the synchronous path is exactly the caller that
    # needs the #86721 self-heal, which has no other startup moment.
    if not session_id:
        return None
    ...
    try:
        ...
        if normalized in {"run", "run_now", "trigger"}:
            ...
            if bg is not None:
                ...
                # The forwarded fire is consumed by the gateway's ticker, so it
                # does NOT carry this turn's manual authority — fail-closed by
                # construction (the attestation is bound to this process's
                # direct-user turn).
