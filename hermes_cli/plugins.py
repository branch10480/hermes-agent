...
def _dispatch_pre_tool_call_hooks(
    ...
    ``direct_user_authority_revision`` / ``direct_user_authority_kind`` carry
    the turn's direct-user provenance through to the hook, exactly as the
    other pre_tool_call wrappers do. They are NOT optional plumbing: every
    dispatch site (``model_tools.handle_function_call``,
    ``agent.tool_executor``, ``agent.agent_runtime_helpers``) passes them and
    swallows a TypeError as "no block", so dropping them here would silently
    disable every pre_tool_call policy plugin instead of failing loudly.
    """
    details = _get_pre_tool_call_directive_details(
        tool_name, args, task_id=task_id, session_id=session_id,
        tool_call_id=tool_call_id, turn_id=turn_id,
