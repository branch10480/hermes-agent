...
def _seed_cron_thread_session(
    ...
    same key the user's reply will resolve to) and append the brief through the
    shipped ``mirror_to_session``. Shared thread sessions ignore ``user_id``;
    per-user thread sessions require the origin user's real ID, otherwise the
    seed lands under ``system:cron`` and the first human reply opens an empty
    sibling session.
    ...
    if not text:
        return
    seed_user_id = str(user_id) if user_id else "system:cron"
    seed_user_name = str(user_name) if user_name else "Cron"
    try:
        ...
        try:
            ...
            platform_enum = None
        # Discord thread destinations must key on the thread's OWN id to match
        # how the Discord adapter keys organic in-thread messages (chat_id ==
        # thread_id). Other platforms (Slack, Telegram) use
        # chat_id == parent_channel for thread messages, so the parent chat_id
        # is correct for them. See the matching guard in
        # GatewayRunner._process_handoff. Hoisted out of the session_store
        # branch: the mirror below must send to the SAME origin chat_id, or it
        # creates the right session row and can never find it again.
        seed_chat_id = (
            str(thread_id)
        ...
        if session_store is not None and platform_enum is not None:
            dest_source = SessionSource(
                ...
                user_name=seed_user_name,
                thread_id=str(thread_id),
                scope_id=str(scope_id) if scope_id else None,
            )
            # Ensure the thread-keyed session row exists so the mirror has a
            # target and the user's later reply joins the same session.
            # Capture the exact id — the mirror writes into THIS row, not an
            # origin-heuristic rediscovery (which bails on populated chats;
            # same class as the flat-seed live failure 2026-08-19).
                    ...
                    if in_channel_surface and inchannel_continuable and not thread_seeded:
                        ...
                        if delivered_message_id:
                            _seed_cron_thread_session(
                                ...
                                is_dm=is_dm_target,
                                scope_id=origin.get("scope_id"),
                                # Same origin user the flat seed above uses:
                                # a per-user thread session seeded under
                                # system:cron is an orphan no human reply
                                # resolves to.
                                user_id=origin_user_id,
                                user_name=origin.get("user_name"),
                            )
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            # Only forward the optional kwargs when they carry a value, so a
            # stubbed/older run_job signature still accepts the call (the same
            # reason upstream branched on ``fire_claim_lost is None``).
            # ``untrusted`` is run_job's own default, so omitting it there is
            # behaviourally identical — it never widens authority.
            if fire_claim_lost is not None:
                run_kwargs["cancel_event"] = fire_claim_lost
            if fire_provenance and fire_provenance != "untrusted":
                run_kwargs["fire_provenance"] = fire_provenance
