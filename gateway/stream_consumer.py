...
class GatewayStreamConsumer:
    ...
    async def _send_commentary(self, text: str) -> bool:
        ...
        try:
            ...
            _md["_interim_send"] = True
            # Commentary is a progress/status send, never conversational
            # content. Discord raises a notification for conversational sends,
            # so stamp it here too (fork 12defc4f5d) — the same marking
            # _non_conversational_metadata() applies on the run.py side.
            _md["non_conversational"] = True
            result = await self.adapter.send(
                chat_id=self.chat_id,
                content=text,
                reply_to=self._initial_reply_to_id,
