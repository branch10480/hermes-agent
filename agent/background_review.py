...
def cancel_background_review_for_live_turn(agent: Any) -> None:
    ...
    A configured idle delay (``auxiliary.background_review.idle_delay_seconds``)
    leaves automatic review work parked in a cancellable ``threading.Timer``
    after the prior turn. Disarm that FIRST: a new user message supersedes
    pending work before it starts, and disarming ahead of the run inspection
    below closes the window where the timer could fire and admit a request
    between the two steps. ``_cancel_background_review_timer`` takes the same
    ``_background_review_lock``, so it must stay outside the ``with`` block —
...
# Historical hardcoded iteration budget for the review fork. Still the default
# for the config-driven budget below; ``_REVIEW_MAX_ITERATIONS_DEFAULT`` is the
# older alias for the same value (see :func:`_review_max_iterations`).
_REVIEW_MAX_ITERATIONS = 16
_REVIEW_MAX_ITERATIONS_DEFAULT = _REVIEW_MAX_ITERATIONS
...
def _review_max_iterations(
    task_cfg: Optional[Dict[str, Any]] = None,
) -> int:
    """Iteration budget for the review fork.

    ``auxiliary.background_review.max_iterations`` (default
    :data:`_REVIEW_MAX_ITERATIONS`). On a slow local model each iteration can
    cost ~a minute of exclusive backend time, so profiles may lower this to
    cut losses earlier; clamped to [1, 100]. ``task_cfg`` is the already-loaded
    ``auxiliary.background_review`` block, so the spawn path does not re-read
    the config file.
    """
    try:
        task = _background_review_task_config(task_cfg)
        raw = int(task.get("max_iterations", _REVIEW_MAX_ITERATIONS))
...
def _run_review_body(
    ...
    Every pre-run gate publishes the run token before returning: the parent's
    :func:`cancel_background_review_for_live_turn` waits on
    ``run.request_done``, so a review that bows out here must still
    ...
    try:
        ...
        with thread_scoped_silence():
            ...
            # uses to interrupt an admitted request). Without this, a review
            # still streaming when the next turn starts races the live turn
            # against the same session_id/credentials — producing doubled
            # prompt-token accounting and a Ctrl+C-proof lockup. The per-review
            # run token separately fences startup and acknowledges request-phase
            # exit; the pointer/list stay best-effort for direct test stubs,
            # with a prepared run token as the live-turn cancellation authority.
            # Agents built without agent_init.py (test stubs) degrade to
            # "no cross-cancellation" rather than aborting the review.
