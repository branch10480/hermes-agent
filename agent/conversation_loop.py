...
_MIDTURN_TOOLS_FROM_AGENT = object()
...
def _midturn_request_pressure_tokens(
    ...
    approx_tokens: int,
    *,
    tools_for_api: Any = _MIDTURN_TOOLS_FROM_AGENT,
) -> int:
    ...
    ``tools_for_api`` is the tool list the *upcoming* request will actually
    carry. It differs from ``agent.tools`` on the answer-only recovery paths
    (reasoning-budget exhaustion / tool-guardrail recovery) which deliberately
    ship an empty toolset — charging their schemas there would inflate the
    pressure figure for tokens that never reach the wire. Callers that do not
    pass it fall back to ``agent.tools``.
    """
    tools = (
        getattr(agent, "tools", None)
        if tools_for_api is _MIDTURN_TOOLS_FROM_AGENT
        else tools_for_api
    )
    try:
    ...
    return approx_tokens + (
        _estimate_tools_tokens_rough(tools) if tools else 0
    )
...
def _run_conversation_core(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # ``tools_for_api`` (not ``agent.tools``) is what this request will
        # carry: the answer-only recovery paths ship an empty toolset, so
        # charging their schemas here would inflate the pressure figure.
