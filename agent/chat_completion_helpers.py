            ...
            if delta_content:
                ...
                if not tool_calls_acc:
                    if pending_text_parts or _provider_stream_text_may_be_sse(delta_content):
                        # Text held back as a possible leaked SSE frame has not
                        # reached the platform yet, so ``stream_delivery_started``
                        # stays False until ``_flush_pending_stream_text`` really
                        # delivers it (the buffer is per stream attempt, so a
                        # retry discards it rather than duplicating it).
            ...
            for event in stream:
                ...
                if event_type == "content_block_start":
                    ...
                    if block and getattr(block, "type", None) == "tool_use":
                        has_tool_use = True
                        # NOTE: deliberately NOT ``stream_delivery_started``.
                        # ``_tool_use_dropped_mid_stream`` below raises
                        # EmptyStreamError for a tool_use block with no
                        # stop_reason, and its contract is that a drop with no
                        # streamed assistant text must surface that error (so
                        # the caller's retry/fallback machinery runs) rather
                        # than degrade to the partial-stream stub — only
                        # streamed *text* flips that gate.  The UnicodeDecodeError
                        # pre-delivery retry guard already blocks on
                        # ``provider_tool_in_flight``/``partial_tool_names``
                        # here, so it does not need this flag on the Anthropic
                        # path.
                        provider_tool_in_flight["yes"] = True
